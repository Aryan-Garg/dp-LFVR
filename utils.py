...
def denormalize1d(x, device='cpu'):
    mean = torch.Tensor([0.456]).view(1, 1, 1, 1).to(device)
    std = torch.Tensor([0.224]).view(1, 1, 1, 1).to(device)
    return x * std + mean
...
def colorize(value, vmin=10, vmax=1000, cmap='magma_r'):
    value = torchvision.utils.make_grid(value)
    #print(value.shape)

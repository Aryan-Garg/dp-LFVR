...
class Inpaint(torch.nn.Module):
	def __init__(self):
		...
		self.objCommon = {'fltBaseline': 40.0, 
						  'fltFocal': 1024 / 2.0,
						  'intWidth': 192*2,
						  'intHeight': 256*2}
	...
	def forward(self, tenImage, tenDisparity, tenShift):
		tenDepth = (self.objCommon['fltFocal'] * self.objCommon['fltBaseline']) / (tenDisparity + 0.0000001)
		tenValid = (spatial_filter(tenDisparity / tenDisparity.max(), 'laplacian').abs() < 0.03).float()
		tenPoints = depth_to_points(tenDepth * tenValid, self.objCommon['fltFocal'])
		tenPoints = tenPoints.reshape(1, 3, -1)

		tenMean = [ tenImage.reshape(tenImage.shape[0], -1).mean(1, True).reshape(tenImage.shape[0], 1, 1, 1), tenDisparity.reshape(tenDisparity.shape[0], -1).mean(1, True).reshape(tenDisparity.shape[0], 1, 1, 1) ]
		tenStd = [ tenImage.reshape(tenImage.shape[0], -1).std(1, True).reshape(tenImage.shape[0], 1, 1, 1), tenDisparity.reshape(tenDisparity.shape[0], -1).std(1, True).reshape(tenDisparity.shape[0], 1, 1, 1) ]
		...
		tenRender, tenExisting = render_pointcloud(tenPoints + tenShift, torch.cat([ tenImage, tenDisparity, tenContext ], 1).view(1, 68, -1), 
											 self.objCommon['intWidth'], self.objCommon['intHeight'], 
											 self.objCommon['fltFocal'], self.objCommon['fltBaseline'])
...
def process_load2(npyImage, objSettings):
	objCommon = {}
	objCommon['fltFocal'] = 192 / 2.0
	...
	disp_img = objSettings['disp']
	disp_img_resized = cv2.resize(disp_img, (objCommon['intWidth'], objCommon['intHeight']), interpolation=cv2.INTER_CUBIC)
	# print(disp_img.shape)
	tenDisparity = torch.from_numpy(disp_img_resized).unsqueeze(0).permute(0, 1, 2).unsqueeze(0).cuda()
	# print(tenDisparity.shape)
	...
	objCommon['tenRawPoints'] = tenPoints.reshape(1, 3, -1)
	objCommon['tenRawUnaltered'] = tenUnaltered.reshape(1, 3, -1)

	objCommon['tenInpaImage'] = objCommon['tenRawImage'].reshape(1, 3, -1)
	objCommon['tenInpaDisparity'] = objCommon['tenRawDisparity'].reshape(1, 1, -1)
	objCommon['tenInpaDepth'] = objCommon['tenRawDepth'].reshape(1, 1, -1)
	objCommon['tenInpaPoints'] = objCommon['tenRawPoints'].reshape(1, 3, -1)
...
if __name__ == '__main__':
	# objFrom = {'fltCenterU': intWidth / 2.0,
	...
	# 			'objFrom': objFrom
	# 			})
	vid_seq = "20230212_131026_246"
	image = "image_213"
	
	disp_imgage = cv2.imread(f"./disparity_maps/disp_pixel4_BA/{vid_seq}/{image}_disp.png", cv2.IMREAD_UNCHANGED)
	disp_imgage = cv2.cvtColor(disp_imgage, cv2.COLOR_BGR2GRAY)
	# print(disp_imgage.shape)
	intWidth, intHeight = 192*2, 256*2

	# Loop here to load Predicted LFs
	npyImage = cv2.imread(f"/data2/raghav/datasets/Pixel4_3DP/rectified/B/Video_data/{vid_seq}/{image}.jpg", cv2.IMREAD_UNCHANGED) 
	npyImage = cv2.resize(src=npyImage, dsize=(intWidth, intHeight), fx=0.0, fy=0.0, interpolation=cv2.INTER_AREA)

	objCommon = process_load2(npyImage, 
							 {"disp": disp_imgage})
	...
	for fltStep in objSettings['fltSteps']:
		...
		tenRenderS.append(tenRender) # Point cloud before inpainting
		tenExistingS.append(tenExisting) # The mask to inpaint
		
		tenRender = fill_disocclusion(tenRender, tenRender[:, 3:4, :, :] * (tenExisting > 0.0).float())

		npyOutput = (tenRender[0, 0:3, :, :].detach().cpu().numpy().transpose(1, 2, 0) * 255.0).clip(0.0, 255.0).astype(np.uint8)
	...
	# npyReconsLightField = npyOutputs.append(npyOutputs)
	print(len(npyOutputS), npyOutputS[0].shape)
	print(len(tenExistingS), len(tenRenderS))

	clip = ImageSequenceClip(npyOutputS, fps=10)
	clip.write_videofile(f'test.mp4', fps=10)

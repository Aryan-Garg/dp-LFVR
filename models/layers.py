...
class PatchTransformerEncoder(nn.Module):
    def __init__(self, in_channels, patch_size=10, embedding_dim=128, num_heads=4):
        ...
        self.positional_encodings = nn.Parameter(torch.rand(1200, embedding_dim), requires_grad=True)
...
class Attention(nn.Module):
    def __init__(self,
        ...
        if cross_attention:
            dim_in_2 = 2*dim_in//5 # 2 out of 5 channels carry DP data
        else:
            dim_in_2 = dim_in
        ...
        # dim_out not actually used in dw_bn method
        self.conv_proj_q = self._build_projection(
            dim_in_2, dim_out, kernel_size, padding_q,
            stride_q, 'linear' if method == 'avg' else method
        )
        self.conv_proj_k = self._build_projection(
            dim_in_2, dim_out, kernel_size, padding_kv,
            stride_kv, method
        )
        ...
        self.proj_q = nn.Linear(dim_in_2, dim_out, bias=qkv_bias)
        self.proj_k = nn.Linear(dim_in_2, dim_out, bias=qkv_bias)
    ...
    def forward_conv(self, x, h, w):
        ...
        if self.cross_attention:
            ...
            x2 = x # FIXME: is this computationally expensive? swap with view?
        if self.conv_proj_q is not None:
            q = self.conv_proj_q(x2)
        else:
            q = rearrange(x, 'b c h w -> b (h w) c')

        if self.conv_proj_k is not None:
            k = self.conv_proj_k(x2)
        else:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        for j in range(depth):
            # Cross attention only for first stage
            # If cross attention mode is turned on
            cross_attention = (j==0) & dp_cross_attention

...
# import simon_softsplat as softsplat
from core.raft import RAFT
from core.utils import flow_viz
from core.utils.utils import InputPadder

RAFT_PT_PATH = './lfvr_weights/raft-things.pth'
...
class SSIMLoss(nn.Module):
    ...
    def forward(self, x, y): # x -> pred ; y -> GT
        x = self.refl(x)
        y = self.refl(y)
        ...
        SSIM_n = (2 * mu_x * mu_y + self.C1) * (2 * sigma_xy + self.C2) # numerator
        SSIM_d = (mu_x ** 2 + mu_y ** 2 + self.C1) * (sigma_x + sigma_y + self.C2) # denominator
...
class StereoPhotometricConsistency(nn.Module):
    def __init__(self, args, device):
        super(StereoPhotometricConsistency, self).__init__()
        self.name = 'Stereoscopic Photometric Consistency Loss'
        self.args = args
    ...
    def forward(self, img, pred_lf, right, left):
        center_pred = pred_lf[:, int(self.angular**2//2), ...]

        reshape_actual_lf = pred_lf.reshape(-1, self.angular, self.angular, 3, img.shape[2], img.shape[3])
        photo_right, photo_left = 0., 0.
        ssim_right, ssim_left = 0., 0.
        if right is not None:
            right_pred = reshape_actual_lf[:, self.angular // 2, -1, ...]
            photo_right = self.diff_loss(right_pred, right)
            ssim_right = self.ssim_loss(right_pred, right)
        if left is not None:
            left_pred = reshape_actual_lf[:, self.angular // 2, 0, ...]
            photo_left = self.diff_loss(left_pred, left)
            ssim_left = self.ssim_loss(left_pred, left)
        
        photo_loss = self.diff_loss(center_pred, img) + photo_right + photo_left
        
        if self.is_ssim:
            photo_loss +=  self.w_ssim * (self.ssim_loss(center_pred, img) + ssim_left + ssim_right)
...
class GeometricConsistency(nn.Module):
    ...
    def forward_warp(self, img, depth):
        ...
        # warped_lf = softsplat.softsplat(tenIn=curr_img, tenFlow=depth, tenMetric=None, strMode='avg')
...
class TemporalConsistency(nn.Module):
    def __init__(self, args, device):
        ...
        self.w_ssim = args.w_ssim
        self.ssim_loss = SSIMLoss().to(device)
        self.flownet = nn.DataParallel(RAFT(args), device_ids=[int(args.gpu)])
        self.flownet.load_state_dict(torch.load(RAFT_PT_PATH))
...
class OcclusionLoss(nn.Module):
    ...
    def forward(self, imgs, depth, pred_lf):
        ...
        return occ_loss, [forward_warp_lf, warped_lfs, mask]#, forward_warp_lf_1]